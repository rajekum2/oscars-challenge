...
Comments- 
    Create a cron scheduler to fetch data from the target.
    Use sling or jcr api to perform crud operations.
    Write a osgi component with property immediate = true and  will specify the path of the newly created folder (i.e /etc/myProject/).
    
    Folder structure-
    
    parent folder (year) will group the movies based on year it was released in repo.
    child level (movie title) will short the movies alphabetical.
...
2. How can you improve the performance of the functionality that you're going to implement?
   Comments-
        ...
        Perform indexing ( lucene )
        URL param based caching ( setup ignoreUrlParam rule at dispatcher level).
        Use TTL (time to live) based caching based on requirement.
